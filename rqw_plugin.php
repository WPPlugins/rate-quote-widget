...
Version: 3.0
Author: Bring the Blog
Author URI: http://ratequotewidget.com/
*/
// ------------------------------------------------------------- //
/**
 * Copyright 2010-2011 | RateQuoteWidget.com | Bring the Blog, LLC
...
function BTB_RQW_CSSource(){
	...
	if(is_string($css = get_option('btbrqw_cssource', null)) and strpos($css = trim($css), '://')) return $css;
	// If the default local file exists, we return it otherwise we return the hosted one
	return BTB_RQW_HOME.'/rqwng.css';
}
/**
 * Show Link?
 * (Function for quick access to a Widget Setting.)
...
function BTB_RQW_PrintWidget($output = true){
	...
	$html = BTB_RQW_FetchURL(BTB_RQW_HOME.'/rqwform.php', array(
		'rqw_apikey'		=> $apikey,
	...
	$html = preg_replace('~<style[^>]*>.+?</style>~si', $customcss, $html);
	$html = str_ireplace('/rqw/rqw.js', '/rqw/rqwng.js', $html);
...
class BTB_RQW_Widget extends WP_Widget{
	...
	const WidgetDescription = 'RateQuoteWidget.com Widget, powered by BringTheBlog, LLC.';
	/**
	 * Widget init code.
	 */
...
class BTB_RQW{
		...
		<p>
			...
			<label class="setting-description" for="BTBRQW_NoWidget">
				Disable <a href="<?php echo admin_url('options-general.php?page=RateQuoteWidget'); ?>"
					target="_blank" style="text-decoration: none;">RateQuoteWidget</a>?
			</label>
		</p>
	<?php }
	/**
	...
	?>
		<div>
			...
			<p>The Plugin is located inside your WordPress folder in <strong><?php echo BTB_RQW_RELPATH; ?>/</strong>.
				Upload you custom CSS files there to style the Widget.</p>
		</div>
	...
	static public function AdminMenu(){
		...
		$page = add_options_page(
			'Rate Quote Widget by Bring the Blog, LLC', 'Rate Quote Widget',
	...
	static public function AdminPageLoad(){
		...
		$_POST['CSSOverride'] = trim($_POST['CSSOverride']);
		update_option('btbrqw_css_override', $_POST['CSSOverride']);
	...
	static public function AdminPage(){ ?>
		...
		<div class="wrap">
			...
			<p>We built Rate Quote Widget to convert leads efficiently and effectively.<br />
				Leveraging proven buyer psychology patterns and advanced lead generation techniques,<br />
				Rate Quote Widget sorts through your inbound leads to find the most ready, willing and <br />qualified customers. Fast.</p>
					...
					<tr>
						...
						<td>
							...
							<label class="setting-description" for="BTB_RQW_WTitle">
								Defaults to <strong>Live Rate Quotes</strong>.
							</label>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							...
							<label class="setting-description" for="BTB_RQW_CSSource">
								<span class="faded">
									You may change the Styling of the Widget by setting a custom URL to the Stylesheet.<br />
								</span>
							</label>
							<div>
								<strong>Remote CSS</strong> <code><?php echo BTB_RQW_HOME.'/rqwng.css'; ?></code> <em>(this is the default hosted Stylesheet)</em>.<br />
								<?php if(($cssfiles = glob(BTB_RQW_PATH.'/*.css')) and !empty($cssfiles)){ ?>
									<p><strong>#<?php echo number_format($count = count($cssfiles)); ?>
										CSS Stylesheet <?php echo ($count > 1) ? 's have' : ' has'; ?></strong> been found in the
					...
					<tr>
						...
						<td>
							...
							<label class="setting-description" for="BTB_RQW_NoClones">
								Make sure the Widget is displayed ONLY ONCE on any given page.<br />
								<em>* Setting is overriden if you manually disable the post on any given page.</em>
							</label>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input type="checkbox" <?php checked(true, BTB_RQW_ShowLink()); ?> name="ShowLink" ID="BTB_RQW_ShowLink" />
			...
			<p>
				...
				You may also paste <code>[ratequotewidget/]</code> in your blog editor to display the widget in a specific page.
			</p>
			<div style="margin: 25px auto; margin-top: 50px; border-top: 1px solid #ddd;">
				<p style="float: right; color: #000; text-shadow: 1px 1px 1px #eee;">&copy; <?php echo date("Y");?>, Bring the Blog, LLC.</p>
				<p>Rate Quote Widget is a <a href="http://bringtheblog.com/?r=rqw" target="_blank">Bring the Blog</a> product.
					You may also like <a href="http://mortga.ge/charts" target="_blank">Real Estate Chart Of The Day</a>.</p>
				<p>Rate Quote Widget requires paid membership.</p>
			</div>
		</div>
	<?php }
};
